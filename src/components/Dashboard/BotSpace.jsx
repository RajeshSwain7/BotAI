...
function BotSpace(props) {
  ...
  const prompts = [
    ...
    },
    {
      ...
      response: "I am doing good, thank you for asking!",
    },
    {
      id: 52,
      question: "Hi, what is the weather",
      response:
        "Its Summer, I would like to connect you with someone who might be better able to answer your question - copy this link and paste it on your browser : https://tronics-weatherapp.vercel.app/",
    },
    {
      id: 51,
    ...
    },
    {
      ...
      response:
        "Its 38 deg, I would like to connect you with someone who might be better able to answer your question - copy this link and paste it on your browser : https://tronics-weatherapp.vercel.app/",
    },
  ];
  const { window } = props;
  ...
  const handleWeather = async () => {
    let city = JSON.parse(localStorage.getItem("geo"));
    try {
      const { data } = await axios.get(
        `https://api.weatherapi.com/v1/forecast.json?key=a3b0fdda2f5540b2a46111734240304&q=${city.city}&days=1&aqi=yes&alerts=yes`
      );
      const { current } = data;
      localStorage.setItem(
        "weather",
  ...
  React.useEffect(() => {
    let now = new Date();
    if (
      getQuestion === "Hi, what is the temperature" &&
      ...
      setQuestionAnswers([
        ...
        {
          ...promtQuestion,
          response: `Temperature based on your location is ${weather.tempC} celcius, ${weather.tempf} fahrenheit`,
      ...
      getQuestion === "Hi, what is my location" &&
      getQuestion.length > 0
    ) {
      let city = JSON.parse(localStorage.getItem("geo"));
      let promtQuestion = promtList.find((x) => x.question === getQuestion);
      setQuestionAnswers([
        ...
        {
          ...promtQuestion,
          response: `Your location is ${city.city}, ${city.country}`,
          time: now.toLocaleString(),
          rating: 0,
          feedBack: "",
        },
      ...
      if (promtQuestion) {
        ...
        setQuestionAnswers([
          ...questionAnswers,
          {
            ...
            response: `I am unable to fulfill this request, kindly ask questions from this list: ${prompts.map(
              (pr, ind) => `${ind + 1})${pr.question}`
            )}`,
            time: now.toLocaleString(),
            rating: 0,
            feedBack: "",
  ...
  const drawer = (
    <div className="d-flex flex-column justify-content-start align-items-center w-100 gap-3">
      ...
      >
        ...
        <TbEdit
          className="editBtn"
          onClick={() => handlePageChange("newPromt")}
        />
      </div>
      <div
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      >
        ...
        {showPage === "defaultPromts" ? (
          <DefaultPromts
            entryText={"How Can I Help You Today?"}
